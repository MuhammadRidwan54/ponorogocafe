            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                    <button onclick="openModal('create')"
                        class="bg-brown-600 hover:bg-brown-700 text-white px-4 lg:px-6 py-2 rounded-lg flex items-center justify-center space-x-2 shadow-md transition-all duration-200 text-sm lg:text-base">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full table-auto">
                    <thead class="bg-gray-50">
                        <tr>
                            <th
                                class="px-3 lg:px-6 py-3 lg:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                No</th>
                            <th
                                class="px-3 lg:px-6 py-3 lg:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Thumbnail</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($cafe as $index => $item)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $index + 1 }}</td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                                    <img src="{{ asset('storage/' . $item->thumbnail) }}"
                                        alt="Thumbnail {{ $item->nama_cafe }}" class="h-16 w-16 object-cover rounded-lg">
                                </td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                                    <div class="flex space-x-2 overflow-x-auto">
                                        @foreach (json_decode($item->gambar) ?? [] as $gambar)
                                            <img src="{{ asset('storage/' . $gambar) }}" alt="{{ $item->nama_cafe }}"
                                ...
                                <td class="px-3 lg:px-6 py-3 lg:py-4 text-sm text-gray-500">
                                    @if ($item->alamat_url)
                                        <a href="{{ $item->alamat_url }}" target="_blank"
                                            class="text-blue-600 hover:underline">{{ $item->alamat_url }}</a>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 text-sm text-gray-500">
                                    {{ $item->fasilitas->pluck('nama_fasilitas')->implode(', ') ?: '-' }}</td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 text-sm text-gray-500">
                                    @if ($item->labels && $item->labels->count())
                                        @foreach ($item->labels as $label)
                                            <span
                                                class="inline-block bg-amber-100 text-amber-800 border border-amber-200 px-2 py-1 rounded-full text-xs mr-1 mb-1">
                                                {{ $label->nama_label }}
                                            </span>
                                        @endforeach
                                    @else
                                ...
                                <td class="px-3 lg:px-6 py-3 lg:py-4 text-sm text-gray-500">
                                    {{ $item->hargamenu->harga_menu ?? '-' }}</td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 text-sm text-gray-500">
                                    {{ $item->kapasitasruang->kapasitas_ruang ?? '-' }}</td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $item->tempatparkir->tempat_parkir ?? '-' }}</td>
                                <td class="px-3 lg:px-6 py-3 lg:py-4 text-sm text-gray-500">
                                    {{ $item->jambuka->jam_buka ?? '-' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="13" class="px-3 lg:px-6 py-3 lg:py-4 text-center text-gray-500">
                                    Belum ada data cafe.
                                </td>
                            </tr>
                        @endforelse
