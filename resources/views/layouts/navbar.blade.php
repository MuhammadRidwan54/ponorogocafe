<!-- Sidebar -->
<div id="sidebar"
    class="bg-brown-800 text-white w-64 min-h-screen p-4 shadow-xl fixed lg:relative transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out z-40">
   ...
   {{ request()->routeIs('fasilitas.*') ? 'bg-brown-700 text-white shadow-md' : 'text-brown-200 hover:bg-brown-700 hover:text-white transition-all duration-200' }}">
        ...
        <a href="{{ route('label.index') }}"
            class="flex items-center space-x-3 p-3 rounded-lg 
           {{ request()->routeIs('label.*') ? 'bg-brown-700 text-white shadow-md' : 'text-brown-200 hover:bg-brown-700 hover:text-white transition-all duration-200' }}">
